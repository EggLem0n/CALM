...
def generate_episode(
    ...
    pickup_points = mapf.walkable_points(env, "pickup_points", walkable_map)
    delivery_points = mapf.walkable_points(env, "delivery_points", walkable_map)
...
class ProgressBox(AnsiBoard):
    """Boxed live progress: agents planned, episodes finished, elapsed and ETA."""
    ...
    def _stats(self) -> List[str]:
        ...
        return [
            ...
            f"[{console.bar(frac, self.BAR_WIDTH)}] {frac * 100.0:5.1f}%",
    ...
    def body(self) -> List[str]:
        return console.boxed(self._stats(), self.INNER_WIDTH)
...
def add_generate_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """Add every dataset-generation flag to `parser` and return it."""
    parser.add_argument(
        "--num_of_process",
        type=int,
...
def validate_generate_args(parser: argparse.ArgumentParser, args) -> None:
    """Reject flag combinations argparse cannot express, via ``parser.error`` (exit 2)."""
...
def run_generation(args) -> None:
    """Generate the dataset for already-parsed `args`.

    Flags left at ``None`` fall back to ``configs/default.yaml`` here rather than in the
    parser, so the YAML stays the single source of truth and ``--help`` does not have to
    read it. ``scripts/02_dataset/generate_dataset.py`` is the launcher.
    """
    base_config = mapf.load_config()  # configs/default.yaml
    ...
    output_dir = run_dir("heatmap_dataset", root=CALM_DATA_DIR).resolve()
    ...
    if workers == 1:                       # in-process run: install the hook ourselves
        _init_progress_hook(counter)       # (the pool initializer also disables Ctrl+C)
    jobs = [(env, episode_id, args, str(output_dir), False)
            for episode_id in range(args.episodes)]
    try:
        with console.live(box):
            for meta, config_template in parallel.imap_unordered(
                    run_episode_job, jobs, workers, star=True,
                    initializer=_init_worker, initargs=(counter,)):
                episode_metas[meta["episode_id"]] = meta
                box.episode_finished(episode_summary(meta["episode_id"], meta))
