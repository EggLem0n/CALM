...
Reads the per-episode shards written by ``calm.dataset`` (``episode_*.npz``)
...
``scripts/02_dataset/render_heatmap.py`` builds the parser from :func:`add_render_args` and
calls :func:`run_render` with the result.
"""
...
def add_render_args(p: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """Add every dataset-render flag to `p` and return it."""
...
def run_render(args) -> None:
    """Render one MP4 per selected episode for already-parsed `args`."""
    ...
    total = len(job_args)
    try:
        for status in parallel.imap_unordered(render_one, job_args, workers, star=True,
                                              initializer=parallel.ignore_sigint):
            done += 1
        ...
        print(f"\nInterrupted; {done}/{total} video(s) finished in {out_dir}", flush=True)
