...
Run (OpenSTL conda env, on the machine that trained):
    python scripts/03_forecast/per_frame_accuracy.py                 # -> reports/.../<stamp>/*.csv + .png
    python scripts/03_forecast/per_frame_accuracy.py --saved <dir>   # point at a copied-over saved/ dir
    python scripts/03_forecast/per_frame_accuracy.py --no-plot       # CSV only (no matplotlib)
"""
...
def add_accuracy_args(ap: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """Add the per-horizon accuracy flags to `ap` and return it."""
...
def run_accuracy(args) -> None:
    """Compute the per-horizon error for already-parsed `args` and write CSV (+ PNG)."""
    ...
    if not (os.path.exists(p_pred) and os.path.exists(p_true)):
        raise SystemExit(
            ...
            "These are written by exp.test() (train.py). Run this on the\n"
    ...
    out_dir = args.out_dir or str(run_dir("congestion_prediction"))
    ...
    if not args.no_plot:
        from calm.forecast.viz.accuracy_curve import plot_curve
