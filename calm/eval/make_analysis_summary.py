"""Generate ``analysis_summary.md`` (+ a copy of ``metrics_glossary.md``) for an evaluation run.

An evaluation run drops ``metrics.csv`` (one row per planner run) and ``metadata.json`` into a
dated report folder. This reads that ``metrics.csv`` and writes, INTO THE SAME FOLDER:
  ...
  * ``analysis_summary.md`` -- a per-run statistical write-up of the congestion-aware effect:
    ...
    incomplete-beta are implemented here so it runs in the pure-numpy evaluation env).
  * ``metrics_glossary.md`` -- the static column glossary, so each run folder is self-contained.

The sweep calls :func:`write_run_reports` when a sweep finishes; you can also regenerate for an
...
def build_analysis_summary(run_dir: Path) -> str:
    ...
    add("")
    # best-lambda-per-gamma (sweep headline), averaged
    blg = []
    for mode in modes:
        for md in mds:
            for g in gammas:
    ...
    if blg:
        add(f"| best-λ per gamma 평균 (sweep 헤드라인) | {np.mean(blg):+.0f} |")
    ...
    add("")
    add("### 재현 (수치 산출 방법)")
    add(f"- 출처: `metrics.csv` ({len(rows)}행). 생성기: `calm/eval/make_analysis_summary.py`.")
