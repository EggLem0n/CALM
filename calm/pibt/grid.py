...
def walkable_points(env: Dict[str, Any], key: str, walkable_map: np.ndarray) -> List[Coord]:
    """``env[key]`` normalised to (x, y) coords, minus any that are not walkable.

    The pickup/delivery pools every caller of ``plan_pibt_repeated_tasks`` has to
    build, in one place.
    """
    return [p for p in normalize_points(env.get(key)) if is_walkable(*p, walkable_map)]
